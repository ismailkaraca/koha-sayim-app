...
const PermissionScreen = ({ onDecision }) => {
    const [testMessage, setTestMessage] = useState({ text: '', type: 'none' });

    const handleCameraTest = async () => {
        if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
            setTestMessage({ text: '❌ Tarayıcınız kamera erişimini desteklemiyor.', type: 'error' });
            return;
        }
        try {
            const stream = await navigator.mediaDevices.getUserMedia({ video: true });
            setTestMessage({ text: '✅ Kamera çalışıyor. Barkod okuma aktif edilecek.', type: 'success' });
            stream.getTracks().forEach(track => track.stop());
        } catch (err) {
            console.error("Kamera testi hatası:", err);
            if (err.name === 'NotAllowedError' || err.name === 'PermissionDeniedError') {
                setTestMessage({ text: '❌ Kamera izni reddedildi. Lütfen tarayıcı ayarlarından izin verin.', type: 'error' });
            } else {
                setTestMessage({ text: '❌ Kamera başlatılamadı. Başka bir uygulama kamerayı kullanıyor olabilir.', type: 'error' });
            }
        }
    };
        ...
        <div className="flex items-center justify-center min-h-screen bg-gray-100">
            <div className="max-w-lg mx-auto p-8 bg-white rounded-xl shadow-lg text-center space-y-6">
                ...
                <div className="p-4 border border-gray-200 rounded-lg space-y-3">
                    <button onClick={handleCameraTest} className="w-full px-6 py-3 bg-gray-200 text-gray-800 font-semibold rounded-md hover:bg-gray-300 transition-colors">
                        Kamerayı Test Et
                    </button>
                    {testMessage.text && <p className={`mt-2 text-sm ${getMessageStyles(testMessage.type)}`}>{testMessage.text}</p>}
                </div>

                <div className="flex flex-col sm:flex-row gap-4">
                    <button onClick={() => handleDecision(true)} className="w-full flex items-center justify-center gap-2 px-6 py-4 bg-green-600 text-white font-bold rounded-md hover:bg-green-700 transition-colors">
                ...
                <p className="text-xs text-gray-500">Bu seçiminiz tarayıcınıza kaydedilecektir.</p>
            </div>
        </div>
    );
};
...
export default function App() {
    ...
    const [page, setPage] = useState('permission'); // Start with permission page
